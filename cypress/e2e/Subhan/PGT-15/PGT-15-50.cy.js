describe('Pengaturan - Tagihan - Pengingat Tagihan - Tambah Pengingat', () => {
  ...
  it('PGT-15.50	Hapus pengingat yang jadwalnya masih aktif → tunggu sampai jadwal masuk	Notifikasi TIDAK terkirim (pengingat sudah dihapus, tidak bisa digunakan lagi)', () => {
    ...
    cy.get('button[role="switch"]').then(($switch) => {
      if ($switch.attr('data-state') === 'unchecked') {
        cy.wrap($switch).click()
      } else {
        cy.log('Switch sudah aktif')
      }
    })
    cy.contains('button', 'Simpan').click()
    cy.wait(1000)
    ...
    cy.wait(2000)
    cy.get('tr:contains("Academy Cazh")').then(($rows) => {
      const total = $rows.length

      Cypress._.times(total, () => {
        cy.get('[data-slot="dialog-trigger"]').eq(0).click()

        cy.contains('button', 'Hapus').click()

        cy.wait(1000) // tunggu request delete selesai
      })
    })
  })
