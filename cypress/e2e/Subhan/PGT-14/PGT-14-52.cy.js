describe('Pengaturan- Tagihan - Jenis Tagihan - Edit Tagihan', () => {
  ...
  it('PGT-14.52	Set status "Aktif" → buka fitur Buat Tagihan & Jadwal Pengaturan Web PPDB	Jenis tagihan tersedia sebagai opsi dropdown di kedua fitur tersebut (bisa digunakan)', () => {
    ...
    cy.get('[role="listbox"]')
      .should('be.visible')
      .contains('[role="option"]', 'Test20')
      .click()
    cy.wait(1500)

    cy.contains('button', 'ANGGOTA').click()
    ...
    cy.wait(1000)
    cy.contains('button', 'Tambah Jadwal').click({ force: true })
    cy.get('[role="dialog"]').within(() => {
      cy.get('[role="combobox"]').eq(1).click()
