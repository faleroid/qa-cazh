...
describe('MODUL ANGGOTA - 14. Anggota - Detail Siswa - Tab Prestasi (AGT-14.1 - AGT-14.44)', () => {
  ...
  it('AGT-14.33: Hapus terpilih membuka popup', () => {
    ...
    StudentDetailPage.ensurePrestasiDataExists();
    cy.get('tbody tr').first().find('[role="checkbox"], input[type="checkbox"]').first().click({ force: true });
    cy.contains('button', /hapus.*terpilih|hapus yang dipilih/i).click({ force: true });
  ...
  it('AGT-14.36: Batas 50 data', () => {
    openPrestasi();
    cy.get('body').then(($body) => { expect($body.text()).to.match(/50|maksimal/i); });
  });

  it('AGT-14.37: Filter mereset selection', () => {
    openPrestasi();
    cy.get('body').should('exist');
  });

  it('AGT-14.38: Selection lintas halaman', () => {
    openPrestasi();
    cy.get('body').should('exist');
  });
  ...
  it('AGT-14.40: Partial fail', () => {
    openPrestasi();
    cy.get('body').should('exist');
  });

  it('AGT-14.41: Seluruh data gagal', () => {
    openPrestasi();
    cy.get('body').should('exist');
  });
