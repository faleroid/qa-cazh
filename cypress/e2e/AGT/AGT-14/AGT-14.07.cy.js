...
describe('AGT-14.7: Cari prestasi dengan keyword apresiasi', () => {
  beforeEach(() => {
    cy.login();
    cy.wait(1000);
  });

  it('AGT-14.7: Cari prestasi dengan keyword apresiasi', () => {
    StudentDetailPage.navigateToFirstStudentDetail();
    StudentDetailPage.clickPrestasiTab();
    ...
    // 2. Lakukan pencarian dengan keyword Apresiasi
    cy.get('input[placeholder*="Cari"], input[type="search"]', { timeout: 15000 })
      .first()
      .clear({ force: true })
      .type(`${data.apresiasi}{enter}`, { force: true });
    cy.wait(1000);

    // 3. Verifikasi hasil pencarian menampilkan data sesuai apresiasi
    cy.get('tbody tr', { timeout: 15000 }).should('contain.text', data.apresiasi);
  });
});
