...
describe('AGT-14.30: Centang header', () => {
  beforeEach(() => { cy.login(); cy.wait(1000); });

  it('AGT-14.30: Centang header', () => { openPrestasi(); cy.get('thead [role="checkbox"], thead input[type="checkbox"]').first().click({ force: true }); cy.get('body').should('exist'); });
});
