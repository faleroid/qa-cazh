...
describe('AGT-14.29: Centang satu baris', () => {
  beforeEach(() => { cy.login(); cy.wait(1000); });

  it('AGT-14.29: Centang satu baris', () => { openPrestasi(); cy.get('tbody tr').first().find('[role="checkbox"], input[type="checkbox"]').first().click({ force: true }); cy.get('body').should(($body) => { expect($body.text()).to.match(/terpilih|dipilih|Hapus/i); }); });
});
