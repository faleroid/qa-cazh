...
describe('AGT-14.18: Upload foto valid', () => {
  beforeEach(() => { cy.login(); cy.wait(1000); });

  it('AGT-14.18: Upload foto valid', () => { openPrestasi(); openForm(); cy.get('[role="dialog"] input[type="file"]').selectFile(`cypress/fixtures/${data.fotoValid}`, { force: true }); cy.get('[role="dialog"]').should('be.visible'); });
});
