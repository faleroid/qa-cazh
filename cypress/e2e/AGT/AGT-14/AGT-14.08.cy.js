...
describe('AGT-14.8: Cari prestasi dengan keyword poin', () => {
  beforeEach(() => {
    cy.login();
    cy.wait(1000);
  });

  it('AGT-14.8: Cari prestasi dengan keyword poin', () => {
    StudentDetailPage.navigateToFirstStudentDetail();
    StudentDetailPage.clickPrestasiTab();
    ...
    // 2. Lakukan pencarian dengan keyword Poin
    cy.get('input[placeholder*="Cari"], input[type="search"]', { timeout: 15000 })
      .first()
      .clear({ force: true })
      .type(`${data.poin}{enter}`, { force: true });
    cy.wait(1000);

    // 3. Verifikasi hasil pencarian menampilkan data sesuai poin
    cy.get('tbody tr', { timeout: 15000 }).should('contain.text', data.poin);
  });
});
