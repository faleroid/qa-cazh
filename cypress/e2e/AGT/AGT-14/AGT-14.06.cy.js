...
describe('AGT-14.6: Cari prestasi dengan keyword deskripsi', () => {
  beforeEach(() => {
    cy.login();
    cy.wait(1000);
  });

  it('AGT-14.6: Cari prestasi dengan keyword deskripsi', () => {
    StudentDetailPage.navigateToFirstStudentDetail();
    StudentDetailPage.clickPrestasiTab();
    ...
    // 2. Lakukan pencarian dengan keyword Deskripsi
    cy.get('input[placeholder*="Cari"], input[type="search"]', { timeout: 15000 })
      .first()
      .clear({ force: true })
      .type(`${data.deskripsi}{enter}`, { force: true });
    cy.wait(1000);

    // 3. Verifikasi hasil pencarian menampilkan data sesuai deskripsi
    cy.get('tbody tr', { timeout: 15000 }).should('contain.text', data.deskripsi);
  });
});
